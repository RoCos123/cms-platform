-- ============================================================================
-- Secțiunea „Pachete": prețuri scrise pe față.
--
-- DE CE ACUM. Se face site-ul de vânzări al platformei, iar acolo prețul
-- trebuie citit în trei secunde: „300 €, primul an inclus, apoi 200 lei/an".
-- Un preț ascuns în spatele lui „cere o ofertă" costă de două ori — cine n-are
-- bugetul scrie oricum și pleacă supărat, iar cine l-ar avea nu scrie deloc,
-- fiindcă nu vrea o conversație ca să afle un număr.
--
-- DE CE E BUNĂ ȘI PENTRU PSIHOLOGI. Era deja pe listă din Faza 0, sub numele
-- „Pachete", pentru pachete de ședințe („5 ședințe la preț redus") — vezi
-- decizii-faza-0.md §6.1. Un cabinet care nu vrea prețuri pe site o stinge, ca
-- pe oricare alta.
--
-- DE CE O MIGRARE, ȘI NU DOAR COD. Panoul n-are „adaugă secțiune": rândurile
-- vin din provizionare. Un tip nou de secțiune n-are cum să ajungă pe un site
-- care există deja — aceeași lecție ca la secțiunea de programare, 27 aug. 2026.
-- ============================================================================

/*
 * Site-urile care există deja o primesc și ele, la aceeași poziție ca la
 * provizionare: între „Programe și materiale" și „Articole recente". Vizibilă,
 * ca toate celelalte — dar goală, iar componenta nu randează nimic fără niciun
 * pachet, deci nu apare o casetă goală pe site-ul nimănui.
 *
 * `not exists` ca migrarea să se poată rula de două ori fără să dubleze nimic.
 */
insert into public.site_content (site_id, key, tone, position, visible, is_demo, data)
select s.id, 'pricing', 'deschis', 78, true, true, '{}'::jsonb
from public.sites s
where not exists (
  select 1 from public.site_content c
  where c.site_id = s.id and c.key = 'pricing'
);

-- ----------------------------------------------------------------------------
-- Provizionarea, din nou: un site nou pornește și cu „Pachete".
-- ----------------------------------------------------------------------------
