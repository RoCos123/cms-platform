 ...
 * dar butonul de descărcare rămânea pe program și ducea la un fișier care nu mai
...
test("ștergerea unui document scoate materialul care-l folosea", () => {
  ...
  const materiale = rezultat.valoare.elemente[0].materiale;
...
test("aceeași funcție scoate și o poză, după uploadId", () => {
  ...
  assert.equal(rezultat.valoare.elemente[0].materiale.length, 2);
});
